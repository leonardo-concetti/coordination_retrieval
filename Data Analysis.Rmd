...
Remove RTs longer than 6000ms (or some other value) (only if I decide they are not meaningful in the generation process of the RTs I'm interested in. The alternative is to model a mixture distribution)
...
Remove RTs shorter than 100ms (or some other value) which are not meaningful data points (they are likely accidental spacebar presses that skipped an ROI before the subject could read it)
...
Take a look at the RTs by condition, with a boxplot
...
Numerically, there is no difference between the accuracy in the two conditions, but there is a difference between the mean RTs at the Matrix Verb. 
...
ggsave(
  ...
  plot = rt_bysubj + theme_minimal() + theme(plot.background = element_rect(fill = "white")),
  dpi = 300,
  width = 25,  
  height = 25
  )
...
*This currently does not work as intended (I want a horizontal line showing the difference between conditions means, on top of the scatterplots)*

```{r, fig.height=25, fig.width=25}

# Calculate the mean RT for each subject and condition separately
mean_rt <- mv |> 
  group_by(subj, condition) |>
  summarise(mean_rt = mean(rt)) |>
  pivot_wider(names_from = condition, values_from = mean_rt)

# Calculate the difference in means between conditions
mean_diff <- mean_rt |>
  mutate(diff = `sing` - `plur`) |>
  select(subj, diff)
...
# Add lines for the mean difference between conditions
rt_bysubj <- rt_bysubj +
  geom_segment(data = mean_rt, aes(x = 1, xend = 2, y = sing, yend = plur), color = "red")
...
Subjects 68, 73, 146, 165, 221 have pretty big variance in their data, so keep that in mind. 
...
Using this custom transform slightly improved the normality of the residuals, but there is no transform that will make them perfect at this point. Considering this and also the fact that nothing changes in terms of significance of the fixed effect of interest, I will stick with the log-transform. 
...
fit_mv_small_post <- brm(rt ~ singplur +
                           (singplur | subj) +
                           (singplur | item),
                         family = lognormal(),
                         prior = c(
...
#plot(fit_mv_small_post)

short_summary(fit_mv_small_post)
...
fit_mv_med_post <- brm(rt ~ singplur +
                         (singplur | subj) +
                         (singplur | item),
                       family = lognormal(),
                       prior = c(
...
#plot(fit_mv_med_post)

short_summary(fit_mv_med_post)
...
fit_mv_large_post <- brm(rt ~ singplur +
                           (singplur | subj) +
                           (singplur | item),
                         family = lognormal(),
                         prior = c(
...
#plot(fit_mv_large_post)

short_summary(fit_mv_large_post)
...
fit_mv_enth_post <- brm(rt ~ singplur +
                          (singplur | subj) +
                          (singplur | item),
                        family = lognormal(),
                        prior = c(
...
#plot(fit_mv_enth_post)

short_summary(fit_mv_enth_post)
...
ppd_mv_small_post <- mcmc_dens(fit_mv_small_post, pars = variables(fit_mv_small_post)[2]) + 
  ggtitle("Normal(0, 0.02)") +
  theme(plot.title = element_text(size = 10))

ppd_mv_med_post <- mcmc_dens(fit_mv_med_post, pars = variables(fit_mv_med_post)[2]) + 
  ggtitle("Normal(0, 0.05)") +
  theme(plot.title = element_text(size = 10))

ppd_mv_large_post <- mcmc_dens(fit_mv_large_post, pars = variables(fit_mv_large_post)[2]) + 
  ggtitle("Normal(0, 1)") +
  theme(plot.title = element_text(size = 10))

ppd_mv_enth_post <- mcmc_dens(fit_mv_enth_post, pars = variables(fit_mv_enth_post)[2]) + 
  ggtitle("Normal(0.06, 0.02)") +
  theme(plot.title = element_text(size = 10))

mv_post_dist <- (ppd_mv_small_post + ppd_mv_med_post) /
  (ppd_mv_large_post + ppd_mv_enth_post)

mv_post_dist

ggsave("mv_post_dist.png", mv_post_dist, width = 6, height = 4, dpi = 300)
...
med_diff_small <- as_draws_df(fit_mv_small_post)[c("b_singplur", "b_Intercept")] |> 
  mutate(
    med_diff = (exp(b_Intercept + b_singplur) - exp(b_Intercept - b_singplur))
  ) |> 
  select(med_diff)

med_diff_med <- as_draws_df(fit_mv_med_post)[c("b_singplur", "b_Intercept")] |> 
  mutate(
    med_diff = (exp(b_Intercept + b_singplur) - exp(b_Intercept - b_singplur))
  ) |> 
  select(med_diff)

med_diff_large <- as_draws_df(fit_mv_large_post)[c("b_singplur", "b_Intercept")] |> 
  mutate(
    med_diff = (exp(b_Intercept + b_singplur) - exp(b_Intercept - b_singplur))
  ) |> 
  select(med_diff)

med_diff_enth <- as_draws_df(fit_mv_enth_post)[c("b_singplur", "b_Intercept")] |> 
  mutate(
    med_diff = (exp(b_Intercept + b_singplur) - exp(b_Intercept - b_singplur))
  ) |> 
  select(med_diff)
...
bayes_factor(fit_mv_small_post, fit_mv_null)
...
bayes_factor(fit_mv_med_post, fit_mv_null)
...
bayes_factor(fit_mv_large_post, fit_mv_null)
...
bayes_factor(fit_mv_enth_post, fit_mv_null)
...
bayes_factor(fit_mv_small_post, fit_mv_enth_post)
...
fit_rcv_small_post <- brm(rt ~ singplur +
                           (singplur | subj) +
                           (singplur | item),
                         family = lognormal(),
                         prior = c(
...
fit_rcv_med_post <- brm(rt ~ singplur +
                         (singplur | subj) +
                         (singplur | item),
                       family = lognormal(),
                       prior = c(
...
fit_rcv_large_post <- brm(rt ~ singplur +
                           (singplur | subj) +
                           (singplur | item),
                         family = lognormal(),
                         prior = c(
...
fit_rcv_enth_post <- brm(rt ~ singplur +
                          (singplur | subj) +
                          (singplur | item),
                        family = lognormal(),
                        prior = c(
...
ppd_rcv_small_post <- mcmc_dens(fit_rcv_small_post, pars = variables(fit_rcv_small_post)[2]) + 
  ggtitle("Normal(0, 0.02)") +
  theme(plot.title = element_text(size = 10))

ppd_rcv_med_post <- mcmc_dens(fit_rcv_med_post, pars = variables(fit_rcv_med_post)[2]) + 
  ggtitle("Normal(0, 0.05)") +
  theme(plot.title = element_text(size = 10))

ppd_rcv_large_post <- mcmc_dens(fit_rcv_large_post, pars = variables(fit_rcv_large_post)[2]) + 
  ggtitle("Normal(0, 1)") +
  theme(plot.title = element_text(size = 10))
