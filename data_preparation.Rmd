...
dat <- read.pcibex("results_pilot.csv")
...
dat <- dat |> 
  select(-Inner.element.number, -PennElementType, -PennElementName,
         ...
         -STUDY_ID, -SESSION_ID, -Order.number.of.item) # Only include "-Newline." in case we removed questions/answers
...
dat <- dat |> 
  select(subj, list, item, condition, roi_number, roi_value, rt, sentence, question, answer, selection, correct)  #remember to add here any other column that you did not remove from the df
